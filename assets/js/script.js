...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        $(".onepage a").on('click', function(e) {
            var hash = this.hash;  // Get the hash (e.g., #about)
        
            if (hash && $(hash).length) {  // Check if the hash exists in the document
                e.preventDefault();  // Prevent default anchor behavior
                var position = $(hash).offset().top;  // Get position
                $("html, body").animate({
                    scrollTop: position
                }, 1000);
            } else {
                console.warn("Section not found:", hash); // Debugging message
            }
        });
